...
const withPrefix = (prefix) => (ext) => (assets, filepath) =>
    Object.values(assets).reduce((acc, name) => {

        // console.log('    ');
        // console.log('    ');
        // console.log('ASSETS: ', Object.values(assets), 'ACC: ', acc, 'NAME: ', name);
        ...
        let chunk = (
            (_.isArray(name)) // ((_.isArray(assets[name]))
                ? name // ? assets[name]
                : [name]
        ).filter(function(filename) { // [assets[name]]).filter(function(filename) {
            // console.log('FILENAME: ', filename, 'EXT:', ext);
            return filename.endsWith(ext) && (filename.search('hot-update') < 0);
        });
            ...
            //   return v;
            // })
            let items = chunk.map(function(filename) { // reduce(function(_, filename) {
